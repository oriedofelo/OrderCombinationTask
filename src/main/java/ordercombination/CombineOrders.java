...
public class CombineOrders {
    ...
    //pick up location distance between points is less than a specific set convenience distance
    if(GeoUtility.coordinatesNearEachOther(x.getPickUpLocation(), y.getPickUpLocation()))
    	return true;
    
    //drop off locations distance between points is less than a specific set convenience distance
        ...
        for (Order order : inputOrders) {
            ...
            while (iterGrp.hasNext()) {
                ...
                while (iter.hasNext()) {
                    ...
                    if (groupOrders(orderItem, order)) {
                        itemFound = true;
